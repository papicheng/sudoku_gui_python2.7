...
class Solve():
    ...
    def _cut_num(self, point):
        ...
        # 所在九宫格(3x3的数组)
        b_r, b_c = [x/3*3 for x in point]  # 九宫格基准点
        for m_r, row in enumerate(self.value[int(b_r):int(b_r+3), int(b_c):int(b_c+3)]):
    ...
    def _check_one_possbile(self):
        # 同一行只有一个数字的情况
        for r in range(0, 9):
            values = [x for x in self.value[r] if isinstance(x, list)]

            for c, item in enumerate(self.value[r]):
                if isinstance(item, list):
                    for value in item:
                        if sum([x.count(value) for x in values]) == 1:
        ...
        # 同一列只有一个数字的情况
        for c in range(0, 9):
            values = [x for x in self.value[:, c] if isinstance(x, list)]
        ...
        for r, c in self.base_points:
            values = [x for x in self.value[r:r+3, c:c+3].reshape(1, -1)[0] if isinstance(x, list)]
    ...
    def _check_same_num(self):
        for b_r, b_c in self.base_points:
            ...
            for i in range(1, 10):
                result = [0 if not isinstance(
                    x[1], list) else x[0] + 1 if x[1].count(i) else 0 for x in enumerate(data)]
                result = [x for x in result if x > 0]
                r_count = len(result)

                if r_count in [2, 3]:
                    # 2或3个元素才有可能同一行或同一列
                    rows = [(x-1) / 3 for x in result]
                    cols = [(x-1) % 3 for x in result]

                    if len(set(rows)) == 1:
                        # 同一行，去掉其他行的数字
                        result = [b_c + (x-1) % 3 for x in result]
                        row = b_r + rows[0]
                        ...
                        # 同一列
                        result = [b_r + (x-1)/3 for x in result]
                        col = b_c + cols[0]
    ...
    def get_num_count(self):
        return sum([1 if isinstance(x, int) else 0 for x in self.value.reshape(1, -1)[0]])
    ...
    def _get_point_score(self, point):
        ...
        if isinstance(item, list):
            score = 10 - len(item)
            score += sum([1 if isinstance(x, int)
                             else 0 for x in self.value[r]])
            score += sum([1 if isinstance(x, int)
                             else 0 for x in self.value[:, c]])
            return score
        else:
            return 0
    ...
    def check_value(self):
        ...
        for row in self.value:
            ...
            if len(set(nums)) != len(nums):
                return False  # 数字要不重复
            if len([x for x in lists if len(x) == 0]):
